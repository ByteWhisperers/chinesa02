...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_user'])) {
        ...
        $active_percentage = $_POST['active_percentage'];

        $pass = password_hash($password, PASSWORD_DEFAULT, array("cost" => 10));

        // Atualizar no banco de dados
        $update_query = "UPDATE usuarios SET mobile='$real_name', token='$token', password='$pass', statusaff='$statusaff', invite_code='$invite', senhaparasacar='$senha_saque', tipo_pagamento = '$tipo_pagamento' , active_percentage = '$active_percentage' WHERE id='" . intval($id_user) . "'";
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane p-3" id="editar" role="tabpanel">
                                <form method="POST" action="">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            <label for="active_percentage" class="form-label">Ativar Porcentagem</label>
                                            <select class="form-select" name="active_percentage" id="active_percentage">
                                                <option value="1" <?= $data['active_percentage'] == 1 ? 'selected' : ''; ?>>Ativo
                                                </option>
                                                <option value="0" <?= $data['active_percentage'] == 0 ? 'selected' : ''; ?>>
                                                    Desativado
                                                </option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            <label for="tipo_pagamento" class="form-label">Tipo De Pagamento Do
                                                Afiliado</label>
                                            <select class="form-select" name="tipo_pagamento" id="tipo_pagamento">
                                                <option value="1" <?= $data['tipo_pagamento'] == 1 ? 'selected' : ''; ?>>
                                                    Cpa
                                                </option>
                                                <option value="0" <?= $data['tipo_pagamento'] == 0 ? 'selected' : ''; ?>>
                                                    Cpa + Rev
                                                </option>
    ...
    `;
                ...
                setTimeout(function () {
                    window.location.href = window.location.href.split("?")[0] + "?phillypsdev=" + encodeURIComponent(new URLSearchParams(window.location.search).get('phillypsdev'));
                }, 3000); // 3 segundos
            }
        ...
        <script>
            ...
            function editarSaldoUsuario(userId, adicionar, remover) {
                fetch('fetch/editar_saldo.php', {
                    ...
                    body: JSON.stringify({ user_id: userId, adicionar: adicionar, remover: remover })
                })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
